...
const FancyActionSheet = ({ isVisible, hide, settings }) => {
  ...
  const sheetStyle = settings.sheetStyle;
  const titleTextStyle = settings.titleTextStyle;
  const messageTextStyle = settings.messageTextStyle;
  const closeButtonStyle = settings.closeButtonStyle;
  const closeButtonIconStyle = settings.closeButtonIconStyle;
  const optionButtonStyle = settings.optionButtonStyle;
  const optionButtonTextStyle = settings.optionButtonTextStyle;
  const destructiveOptionButtonStyle = settings.destructiveOptionButtonStyle ;
  const destructiveOptionButtonTextStyle = settings.destructiveOptionButtonTextStyle;
  const options = settings.options ?? [];
  const destructiveOptionId = settings.destructiveOptionId;
  const onOptionPress = settings.onOptionPress;
  return (
    <Modal
      transparent={true}
      visible={isVisible}
        ...
        <View style={[styles.sheetView, { ...sheetStyle }]}>
          ...
          <View style={[styles.optionButtonsView]}>
            {options.map((option) => {
              ...
              return (
                <Pressable style={[styles.optionButton, { ...(isDestructiveOption ? destructiveOptionButtonStyle : optionButtonStyle) }]} onPress={() => { onOptionPress(option); hide(); }}>
          ...
          <TouchableOpacity style={[styles.closeButton, { ...closeButtonStyle }]} onPress={hide} aria-label='Close'>
